...
namespace Unity.WebRTC
{
    ...
    public static class WebRTC
    {
        ...
        internal const string Lib = "Assets/Runtime/Plugins/x86_64/webrtc.dll";
...
#else
            NativeMethods.RegisterDebugLog(LogLevel, DebugLog);
#endif
...
#else
            NativeMethods.RegisterDebugLog(DebugLogLevel.None, null);
#endif
#if UNITY_EDITOR
            UnityEditor.AssemblyReloadEvents.beforeAssemblyReload -= OnBeforeAssemblyReload;
#endif
...
#endif
        public static void WriteLine(DebugLogLevel logLevel, string message,
        [CallerFilePath] string file = "",
        [CallerLineNumber] int line = 0,
        [CallerMemberName] string member = "")
        {
            if (((int)LogLevel > 0 && (int)LogLevel <= 3 && (int)logLevel > 0 && (int)logLevel <= (int)LogLevel) ||
                ((int)LogLevel > 3 && (int)LogLevel <= 6 && (int)logLevel > 3 && (int)logLevel <= (int)LogLevel))
            {
                var s = string.Format("[CS] {0}:{1} - {2}: {3}", Path.GetFileName(file), line, member, message);
                if (logLevel == DebugLogLevel.Error || logLevel == DebugLogLevel.CSOnly_Error) Debug.LogError(s);
                if (logLevel == DebugLogLevel.Warning || logLevel == DebugLogLevel.CSOnly_Warning) Debug.LogWarning(s);
                if (logLevel == DebugLogLevel.Log || logLevel == DebugLogLevel.Log) Debug.Log(s);
            }
        }
...
#else
        [DllImport(WebRTC.Lib)]
        public static extern IntPtr ContextCreateVideoTrack(IntPtr ptr, IntPtr srcTexturePtr, IntPtr dstTexturePtr, int width, int height);
        [DllImport(WebRTC.Lib)]
        public static extern IntPtr ContextCreateAudioTrack(IntPtr ptr);
#endif
        [DllImport(WebRTC.Lib)]
...
#else
        [DllImport(WebRTC.Lib)]
        public static extern IntPtr ContextGetSenderCapabilities(IntPtr context, TrackKind kind);
#endif
#if !UNITY_WEBGL
        [DllImport(WebRTC.Lib)]
...
#else
        [DllImport(WebRTC.Lib)]
        public static extern IntPtr ContextGetReceiverCapabilities(IntPtr context, TrackKind kind);
#endif
        [DllImport(WebRTC.Lib)]
...
#else
        [DllImport(WebRTC.Lib)]
        public static extern IntPtr PeerConnectionGetReceivers(IntPtr ptr);
        [DllImport(WebRTC.Lib)]
        public static extern IntPtr PeerConnectionGetSenders(IntPtr ptr);
        [DllImport(WebRTC.Lib)]
        public static extern IntPtr PeerConnectionGetTransceivers(IntPtr ptr);
#endif
        [DllImport(WebRTC.Lib)]
